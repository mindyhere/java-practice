...
public class ScoreDAO {
	...
	public int update_score(ScoreDTO dto) {
		...
		try {
			conn = DB.dbConn();
			String sql = "update scores set name = ?, kor = ?, eng = ?, mat= ?,tot = ?, avg = ? "
					+ "where student_no = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public Vector search_score(String name) {
		...
		try {
			...
			String sql = "select student_no, name, kor, eng, mat, (kor+eng+mat) tot, "
					+ "round(((kor+eng+mat)/3.0),1) avg from scores where name like ? order by name";
			pstmt = conn.prepareStatement(sql);
	...
	public Vector list_score() {
		...
		try {
			conn = DB.dbConn();
			String sql = "select student_no, name, kor, eng, mat, (kor+eng+mat) tot, "
					+ "round(((kor+eng+mat)/3.0),1) avg from scores order by name";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
